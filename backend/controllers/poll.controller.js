...
const generateResult = async (req, res) => {
  try {
    ...
    const existingResult = await Result.findOne({ pollId: poll._id });
    if (existingResult) {
      return res.status(200).json({
        message: "Result already generated for this poll",
    ...
    res.status(201).json({ message: "Result generated successfully", result });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: "Internal server error" });
  }
